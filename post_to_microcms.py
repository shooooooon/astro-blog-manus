#!/usr/bin/env python3
"""
microCMS APIを使用してManusの自己紹介記事を自動投稿するスクリプト
"""
...
def upload_image_to_microcms(image_path):
    """画像をmicroCMSにアップロード"""
    ...
    # microCMS メディアAPI用のペイロード
    media_payload = {
        "name": "blog-automation-with-manus.jpg",
    ...
    try:
        response = requests.post(media_url, headers=headers, json=media_payload)
        response.raise_for_status()
        
        media_data = response.json()
        print(f"画像アップロード成功: {media_data['url']}")
        return media_data
...
def create_blog_post(image_data):
    """ブログ記事をmicroCMSに投稿"""
    print("ブログ記事を投稿中...")
...
def main():
    ...
    # 画像パス
    image_path = "/home/ubuntu/astro-blog-manus/public/images/blog-automation-with-manus.jpg"
    ...
    # 1. 画像をアップロード
    image_data = upload_image_to_microcms(image_path)
    if not image_data:
        print("画像アップロードに失敗しました。記事投稿を中止します。")
        return
    
    # 2. ブログ記事を投稿
    blog_data = create_blog_post(image_data)
    if blog_data:
        print("\n=== 投稿完了 ===")
